...
export default function ScrapingConfigPanel({
  ...
  const configuredPlatforms = scrapingConfigs.map((c) => c.platform);
  const missingPlatforms = (sector?.platforms ?? []).filter(
    (p) => !configuredPlatforms.includes(p)
  );

  async function triggerScraping(config: ScrapingConfig) {
    const key = `${locationId}:${config.platform}`;
    if (triggerLoading === key) return;

    const currentConfig = scrapingConfigs.find((item) => item.id === config.id) ?? config;
    ...
    if (error) {
      ...
      onConfigsChange(scrapingConfigs.map((item) =>
        item.id === config.id
  ...
  async function triggerLocationPipeline() {
  ...
  async function saveScrapingConfigs() {
    if (!sector) return;

    setConfigSaving(true);

    const canAdd = (p: string) =>
      sector.platforms.includes(p) && !configuredPlatforms.includes(p);
    ...
    if (error) {
      ...
      onConfigsChange([...scrapingConfigs, ...data]);
  ...
  return (
    <div class="mt-2 space-y-2">
      {/* Action bar: CSV download + AI pipeline */}
      <div class="flex items-center gap-2 mb-1">
        <button
          ...
          onClick={async () => {
            setCsvLoading(true);
            try {
              await downloadLocationCsv(supabase, locationId, locationName, businessId);
              onMessage({ type: "ok", text: `CSV scaricato per "${locationName}"` });
            } catch {
              onMessage({ type: "err", text: `Errore download CSV per "${locationName}"` });
            }
            setCsvLoading(false);
          }}
        ...
        >
          {csvLoading ? "..." : "Download recensioni"}
        </button>
        <button
          type="button"
          disabled={analysisLoading}
          onClick={() => triggerLocationPipeline()}
        ...
        >
          {analysisLoading ? "..." : "Avvia Pipeline AI"}
        </button>
      </div>

      {/* Existing configs */}
      {scrapingConfigs.map((cfg) => {
        const st = STATUS_LABELS[cfg.status] ?? STATUS_LABELS.idle;
        const key = `${locationId}:${cfg.platform}`;
      ...
      {configuringOpen ? (
        <div class="rounded-lg border border-blue-200 bg-blue-50 p-4 space-y-4">
          <div class="flex items-center justify-between">
            <h3 class="text-sm font-bold text-gray-700">
              {scrapingConfigs.length > 0 ? "Aggiungi piattaforma" : "Configura Piattaforme"}
            </h3>
            <button
              type="button"
              onClick={() => {
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => setConfiguringOpen(true)}
